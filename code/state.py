...
class State():
    ...
    def realizeDemand(self, new_node, dist, connected_sup, G_collapsed, Cost,reachable_nodes):
        ...
        if  len(connected_sup)== 0:
            ...
            Cost, check = funcs.updateCost(new_node, connected_sup, demand_nodes, G_collapsed, Cost)
            if check ==1: #If nothing has changed - no need to make any calculations
                demand_vec, supply_vec, satisfied_demand = funcs.supplyAllocation(Cost, self.rem_demand, self.rem_supply, demand_nodes, supply_nodes)
            else:
                satisfied_demand = 0
                demand_vec = self.rem_demand
                supply_vec = self.rem_supply

        return demand_vec, supply_vec, satisfied_demand
